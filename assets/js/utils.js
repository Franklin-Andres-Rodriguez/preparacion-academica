  ...
  window.AppUtils = {
    ...
    dom: {
      ...
      $$(selector, context = document) {
        try {
          ...
          console.error(`[AppUtils.dom] Invalid selector for multiple: ${selector}`, error);
          return [];
        }
      },
    ...
    */
    ...
    education: {
      ...
      calculateProgress(completed, total, weights = null) {
        if (!this.validate.isNumber(completed) || !this.validate.isNumber(total) || total === 0) {
      ...
      calculateLearningVelocity(sessions) {
        if (!this.validate.isArray(sessions) || sessions.length < 2) {
          return { velocity: 0, trend: 'insufficient_data', recommendation: 'complete_more_sessions' };
        }
      ...
      assessCompetency(responses, correctAnswers, bloomsLevels = []) {
        if (!this.validate.isArray(responses) || !this.validate.isArray(correctAnswers)) {
      ...
      generateLearningFeedback(results, percentage) {
        ...
        if (percentage >= 90) {
          feedback.push('🎉 Excelente dominio! Considera avanzar al siguiente nivel.');
        } else if (percentage >= 75) {
          feedback.push('👍 Buen progreso. Refina algunos conceptos clave.');
        } else if (percentage >= 60) {
          feedback.push('📚 En desarrollo. Practica más los conceptos fundamentales.');
        } else {
        ...
        if (incorrectAnswers.length > 0) {
          const bloomsErrors = this.data.groupBy(incorrectAnswers, 'bloomsLevel');
          Object.keys(bloomsErrors).forEach(level => {
            feedback.push(`💡 Refuerza habilidades de ${level}: ${bloomsErrors[level].length} errores`);
          });
        }
    ...
    debug: {
      ...
      inspect(obj, maxDepth = 3, currentDepth = 0) {
        ...
        if (Array.isArray(obj)) {
          return obj.map(item => this.inspect(item, maxDepth, currentDepth + 1));
        }
    ...
    */
    ...
    storage: {
      ...
      clear() {
        try {
          const keys = Object.keys(localStorage).filter(key => key.startsWith('mdb_'));
          ...
          const observer = new PerformanceObserver((list) => {
            list.getEntries().forEach(entry => {
              if (entry.duration > 50) { // Long task > 50ms
                this.debug.log('warn', `Long task detected: ${Math.round(entry.duration)}ms`);
              }
            });
          });
